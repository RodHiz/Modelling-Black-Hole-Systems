...
Kerr black hole geodesic integration using first-order separated equations
...
def compute_tdot_phidot(r, theta, a, E, Lz):
    """
    From conserved E and Lz, compute dt/dtau and dphi/dtau.
    P(r) = E*(r^2+a^2) - a*Lz
    """
    sig = Sigma_func(r, theta, a)
    delta = Delta_func(r, a)
    s2 = np.sin(theta)**2
...
def R_potential(r, a, E, Lz, Q_carter):
    """Radial potential R(r). p_r^2 = R(r)."""
    delta = Delta_func(r, a)
    P = E * (r**2 + a**2) - a * Lz
    K = (Lz - a*E)**2 + Q_carter
...
a = 0.9      # Spin parameter (0 = Schwarzschild, <1 for sub-extremal)

# Event horizons
r_plus  = 1 + np.sqrt(1 - a**2)
r_minus = 1 - np.sqrt(1 - a**2)
print(f"=== Kerr Black Hole ===")
print(f"Spin a = {a}")
...
# Circular orbit values for Kerr (prograde equatorial)
...
print(f"\nCircular orbit values at r={r0}:")
print(f"  E_circ = {E_circ:.6f}")
print(f"  L_circ = {L_circ:.6f}")
...
tdot0, phidot0 = compute_tdot_phidot(r0, theta0, a, E_init, Lz_init)

# Enforce metric norm = -1 by adjusting tdot0
sig0 = Sigma_func(r0, theta0, a)
delta0 = Delta_func(r0, a)
s2_0 = np.sin(theta0)**2

g_tt = -(1 - 2*r0/sig0)
g_tphi = -2*a*r0*s2_0/sig0
g_phiphi = (r0**2 + a**2 + 2*r0*a**2*s2_0/sig0) * s2_0
...
p_r0 = sig0 * rdot0
p_th0 = sig0 * thetadot0
R_check = R_potential(r0, a, E_init, Lz_init, Q_init)
...
def f_kerr(state):
    """
    Equations of motion for Kerr geodesics (first-order separated form).
    ...
    tdot, phidot = compute_tdot_phidot(r, theta, a, E_init, Lz_init)
    rdot   = p_r / sig
    thdot  = p_th / sig

    # Momentum equations
    dp_r_dtau  = dR_dr(r, a, E_init, Lz_init, Q_init) / (2.0 * sig)
...
while run_sim:
    ...
    newstate, dt_new, accepted = rkf45(f_kerr, oldstate, dt_now)
...
for i in range(len(rpath)):
    tdotpath[i], phidotpath[i] = compute_tdot_phidot(rpath[i], thetapath[i], a, E_init, Lz_init)
...
# Kerr metric components along trajectory
g_tt_path    = -(1 - 2*rpath/sigpath)
g_tphi_path  = -2*a*rpath*s2path/sigpath
g_rr_path    = sigpath / deltapath
g_thth_path  = sigpath
g_phiphi_path = (rpath**2 + a**2 + 2*rpath*a**2*s2path/sigpath) * s2path

# Energy: E = -(g_tt*tdot + g_tphi*phidot)
E_check = -(g_tt_path * tdotpath + g_tphi_path * phidotpath)

# Angular momentum: Lz = g_tphi*tdot + g_phiphi*phidot
Lz_check = g_tphi_path * tdotpath + g_phiphi_path * phidotpath
...
# Potential consistency check
R_check_arr = np.array([R_potential(rpath[i], a, E_init, Lz_init, Q_init) for i in range(len(rpath))])
...
axes_raw[1].set_title('Angular momentum (z)')
...
# ============ 3D PLOT ======================================================
fig = plt.figure('3D Kerr Geodesic', figsize=(10, 10))
...
r_ergo = 1 + np.sqrt(1 - a**2 * np.cos(THETA_e)**2)
...
ax.set_ylabel('Y')
ax.set_zlabel('Z')
ax.set_title(f'3D Geodesic: Kerr (a={a})', fontweight='bold')
